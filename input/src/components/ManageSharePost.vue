              ...
              <div v-for="(post,index) in posts" :key="index">
                <div class="message_field">
                    <div style="height: 100%;">
                        ...
                        <div id="sort_box" style="width: 10%;">
                            <button @click="remove_post(post.postVo.post.id)">删除</button>
                            <span style="cursor: pointer;" @click="pass(post.postVo.post.id)">忽略</span>
                        </div>
                    </div>
                </div>
              </div>
...
export default {
   ...
   methods:{
       find(){
            console.log("查看举报帖子信息");
            axios.get('https://i5101b0918.oicp.vip/shareAdmin/findPostReport',{
            params:{
                    },
            })
            .then((res)=>{
       ...
       remove_post(id){
           if(confirm("确认删除该贴？")){
            console.log("删除违规帖子");
                this.$http({
                method: "post",
                url: "/shareAdmin/deleteIllegalPost",
                data: {
                    postId:id//被删除的帖子的id
                },
                }).then(
